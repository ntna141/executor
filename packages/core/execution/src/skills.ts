...
const EXECUTE_SKILL_BODY = [
  ...
  "## Workflow",
  "",
  "1. Before calling `execute`, plan the complete data flow. For a read-only lookup, default to one `execute` call for discovery, API calls, filtering, and result construction.",
  '2. Find the needed paths with `tools.search({ query: "<intent + key nouns>", limit: 12 })`. Batch independent searches with `Promise.all`.',
  "3. Read a shape with `tools.describe.tool({ path })` only when the input is unclear. Do not use a separate `execute` call only to return search results or schemas.",
  "4. Keep dependent operations in the same program with sequential `await`; batch independent API calls with `Promise.all`.",
  "5. Call discovered paths with `tools[path](input)`, check each result, and return one compact object containing only the evidence needed for the answer.",
  "6. Use a second `execute` call only after a real error or insufficient result. Do not try speculative query variants without evidence.",
  "7. Writes, approvals, and user interactions may require separate executions. For live saved-connection inventory, call `tools.executor.coreTools.connections.list({})`; after checking `result.ok`, read `result.data.connections`.",
  "",
  "## Rules",
  "",
  ...
  "- For read-only work, return combined, compact results. Omit full schemas, HTTP headers, and unused collection fields unless the user needs them.",
  "- Resolve relative dates in the user's known timezone. Do not use a UTC date as the user's local `today`.",
  ...
  "## Slack fast path",
  "",
  "When Slack is connected as `slack.user.slack`, the following common tools are already known. For routine Slack work, do not call `tools.search()` or `tools.describe.tool()` for them. Put discovery, dependent Slack calls, normalization, deduplication, sorting, and evidence construction in one `execute` program.",
  "",
  "```ts",
  "slack_search_users({ query: string, limit?: number })",
  'slack_search_public({ keywords?: string[], filters?: string, limit?: number, sort?: "score" | "timestamp", sort_dir?: "asc" | "desc" })',
  'slack_search_public_and_private({ keywords?: string[], filters?: string, limit?: number, sort?: "score" | "timestamp", sort_dir?: "asc" | "desc" })',
  "slack_search_channels({ query?: string, limit?: number })",
  "slack_read_channel({ channel_id: string, limit?: number })",
  "slack_read_thread({ channel_id: string, message_ts: string, limit?: number })",
  "```",
  "",
  "User search returns `data.users` with `id`, `name`, `real_name`, and `profile`. Message search returns `data.results` with fields such as `user`, `username`, `ts`, `text`, `channel`, and `permalink`.",
  "",
  "Match retrieval breadth to the request. For one newest item, sort by timestamp descending and select the first valid result. For a summary, research question, or broad request, cast wide: run useful query variants concurrently, use the available result window or pagination, merge results, deduplicate by permalink or timestamp, and sort them. Do not impose an arbitrary small limit.",
  "",
  "Resolve a display name and search its messages in the same program. Do not first try a guessed Slack username. Example shape:",
  "",
  "```ts",
  "const slack = tools.slack.user.slack;",
  "const user = await slack.slack_search_users({ query: person });",
  "if (!user.ok) return user;",
  "const username = user.data.users?.[0]?.name;",
  "if (!username) return { error: `Slack user not found: ${person}` };",
  "const messages = await slack.slack_search_public_and_private({",
  "  filters: `from:${username}` ,",
  '  sort: "timestamp",',
  '  sort_dir: "desc",',
  "});",
  "if (!messages.ok) return messages;",
  "return messages.data.results.map((message) => ({",
  ...
  "```",
  "",
  'For summaries, return one evidence corpus from the program, for example `evidence.map(format).join("\\n\\n")`. The model summarizes that single returned corpus. Use `tools.search()` only for uncommon Slack actions, a different Slack connection address, or another integration.',
].join("\n");

export const EXECUTE_SKILL: Skill = {
  name: "execute",
  summary:
    "How to batch integration discovery, calls, and compact results in the execute sandbox, emit content, and resume paused runs.",
